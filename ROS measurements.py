# -*- coding: utf-8 -*-
"""
Created on Sat Jul 20 17:04:06 2019
...
# make a list for ROSmeasures to be attached to
    ROSIntensityList = [['filename', 'label number', 'mean intensity', 'min intensity', 'max intensity', 'area of bacteria']]
# for each label in ROSmeasures, add to ROSIntensityList
    for label in range(0, delfilt_num_objects2):
        labelmeasure = (filename, label, ROSmeasures[label].mean_intensity, ROSmeasures[label].min_intensity, ROSmeasures[label].max_intensity, ROSmeasures[label].area)
